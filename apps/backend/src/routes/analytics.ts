...
export async function invalidateAnalyticsCache(orgId: string, campaignId?: string): Promise<void> {
  const keys = [`analytics:stats:${orgId}`, `analytics:campaigns:${orgId}`];
  if (campaignId) keys.push(`analytics:ab:${campaignId}`);
...
router.get('/stats', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const data = await getCached(`analytics:stats:${orgId}`, 300, async () => {
...
router.get('/campaign/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const data = await getCached(`analytics:campaigns:${orgId}`, 300, async () => {
...
router.get('/ab/:campaignId', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const data = await getCached(`analytics:ab:${campaign.id}`, 300, async () => {

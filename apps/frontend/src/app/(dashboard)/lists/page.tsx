...
export default function ListsPage() {
  ...
  async function listFieldMove(recordId: string, _from: string | null, to: string | null) {
    ...
    try {
      ...
      showToast(to ? 'Moved · list field saved on the entry' : 'Cleared · list field saved on the entry');
    } catch (e) {
      const ax = e as { response?: { data?: { error?: string } } };
      showToast(ax?.response?.data?.error ?? 'Move failed');
      if (selectedId) await loadDetail(selectedId);
  ...
  async function saveCell(recordId: string, attr: CrmListAttribute, raw: unknown) {
    ...
    try {
      ...
      const ax = e as { response?: { data?: { error?: string } } };
      showToast(ax?.response?.data?.error ?? 'Failed to save list field');
    } finally {
      setCellSaving(false);
    }
  }
  ...
  async function addListField() {
    ...
    try {
      ...
      showToast('List field added');
    } catch (e) {
      const ax = e as { response?: { data?: { error?: string } } };
      showToast(ax?.response?.data?.error ?? 'Failed to add list field');
    } finally {
      setNfSaving(false);
    }
  }
  ...
  function renderListCell(r: ListRecordEntry, a: CrmListAttribute): ReactNode {
    ...
    if (a.type === 'BOOLEAN') {
      ...
      return (
        <button type="button" disabled={cellSaving} onClick={() => void saveCell(r.id, a, !checked)} className={`inline-flex h-6 items-center rounded-md px-1.5 text-[11px] font-semibold ${checked ? 'bg-emerald-50 text-emerald-700' : 'bg-surface-2 text-ink-subtle'} hover:ring-1 hover:ring-indigo-200`}>{checked ? 'Yes' : 'No'}</button>
      );
    }
    ...
    return (
      <button type="button"
        ...
        {disp ? <span className="truncate" title={disp}>{disp}</span> : <span className="text-[11px] text-indigo-400">+ add</span>}
      </button>
    );
  }
  ...
  async function saveAsNewView() {
    ...
    try {
      ...
      const ax = e as { response?: { data?: { error?: string } } };
      showToast(ax?.response?.data?.error ?? 'Could not save view');
  ...
  async function updateActiveView() {
    ...
    try {
      ...
      showToast(`View “${v.name}” updated`);
    } catch (e) {
      const ax = e as { response?: { data?: { error?: string } } };
      showToast(ax?.response?.data?.error ?? 'Could not update view');
    } finally { setViewBusy(false); }
  }
  ...
  async function removeView(v: CrmView) {
    ...
    } catch { showToast('Could not delete view'); }
  }
    ...
    <>
            ...
            <div className={`mx-auto p-6 ${listViewType === 'board' ? 'max-w-[1600px]' : 'max-w-4xl'}`}>
              ...
              {!restricted && (
                <div className="mb-2.5 flex flex-wrap items-center gap-1.5">
                  <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-[0.12em] text-ink-subtle"><Bookmark size={11} /> Views</span>
                  <button type="button" onClick={clearActiveView} className={['inline-flex h-7 items-center rounded-full border px-2.5 text-[12px] font-semibold transition-colors', !activeViewId ? 'border-brand-400 bg-brand-50 text-brand-700' : 'border-line bg-surface text-ink-muted hover:bg-surface-2'].join(' ')}>Default</button>
                  {savedViews.map((v) => (
                    <span key={v.id} className={['group inline-flex h-7 items-center gap-1 rounded-full border pl-2.5 pr-1.5 text-[12px] font-semibold transition-colors', activeViewId === v.id ? 'border-brand-400 bg-brand-50 text-brand-700' : 'border-line bg-surface text-ink-muted hover:bg-surface-2'].join(' ')}>
                      <button type="button" onClick={() => selectView(v)} className="inline-flex items-center gap-1">
                        ...
                        {activeViewId === v.id && activeDirty && <span className="ml-0.5 h-1.5 w-1.5 rounded-full bg-amber-500" title="Unsaved changes" />}
                      </button>
                      <button type="button" onClick={() => void removeView(v)} title="Delete view" className="flex h-5 w-5 items-center justify-center rounded-full text-ink-subtle opacity-0 transition-opacity hover:bg-rose-50 hover:text-rose-600 group-hover:opacity-100"><X size={11} /></button>
                    </span>
                  ))}
                  ...
                  {!saveViewOpen ? (
                    <button type="button" onClick={() => { setSaveViewOpen(true); setSaveViewName(activeView ? `${activeView.name} copy` : ''); }} className="inline-flex h-7 items-center gap-1 rounded-full border border-dashed border-line px-2.5 text-[12px] font-semibold text-brand-700 hover:bg-brand-50"><Plus size={12} /> Save as view</button>
                  ) : (
                    <span className="inline-flex items-center gap-1 rounded-full border border-brand-200 bg-brand-50/40 px-1.5 py-0.5">
                      <input autoFocus value={saveViewName} onChange={(e) => setSaveViewName(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') void saveAsNewView(); else if (e.key === 'Escape') setSaveViewOpen(false); }} placeholder="View name" className="h-6 w-32 rounded-md border border-line bg-surface px-2 text-[12px] text-ink focus:border-brand-400 focus:outline-none" />
                      <button type="button" disabled={viewBusy || !saveViewName.trim()} onClick={() => void saveAsNewView()} className="inline-flex h-6 items-center gap-1 rounded-md bg-brand-600 px-2 text-[12px] font-semibold text-white hover:bg-brand-700 disabled:opacity-50">{viewBusy ? <Loader2 size={11} className="animate-spin" /> : <Check size={11} />} Save</button>
                      <button type="button" onClick={() => { setSaveViewOpen(false); setSaveViewName(''); }} className="inline-flex h-6 items-center rounded-md border border-line px-2 text-[12px] font-semibold text-ink-muted hover:bg-surface-2">Cancel</button>
                    </span>
                  )}
                </div>
              )}
              ...
              <div className="mb-2.5 flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  {!isPipeline && listViewType === 'board' && groupChoices.length > 0 && (
                    <select value={groupByKey} onChange={(e) => setGroupByKey(e.target.value)} title="Group board by — ◇ marks a list-specific field" className="h-8 rounded-lg border border-line bg-[var(--surface)] px-2 text-[12px] font-medium text-ink-muted focus:border-brand-400 focus:outline-none">
                      {groupChoices.map((g) => <option key={g.key} value={g.key}>by {g.label}</option>)}
                    </select>
                  )}
                  {isPipeline && (
                  ...
                  {isDynamic ? (
                    ...
                    <>
                      ...
                      <button type="button" onClick={() => setImportOpen(true)} title="Import records from a CSV into this list (object + ◇ list fields)" className="inline-flex h-8 items-center gap-1.5 rounded-lg border border-line bg-surface px-2.5 text-[12.5px] font-semibold text-ink-muted hover:bg-surface-2 transition-colors"><Upload size={14} /> Import CSV</button>
              ...
              {showFilter && (
                <div className="mb-2.5 rounded-2xl border border-line bg-surface p-3 shadow-sm">
                  <p className="mb-1.5 text-[11px] font-bold uppercase tracking-[0.06em] text-ink-subtle">Advanced filter · AND / OR groups · <span className="text-indigo-600">◇ = list field</span></p>
              ...
              {!isDynamic && (
                <div className="mb-1.5 flex flex-wrap items-center gap-2">
                  <span className="inline-flex items-center gap-1 rounded-md bg-indigo-50 px-1.5 py-0.5 text-[10.5px] font-bold uppercase tracking-wide text-indigo-700"><Tag size={11} /> List fields</span>
                  <span className="text-[11.5px] text-ink-subtle">{listFields.length ? `${listFields.length} field${listFields.length === 1 ? '' : 's'} live on this list — not on the records` : 'No list-specific fields yet'}</span>
                  {!addFieldOpen ? (
                    <button type="button" onClick={() => setAddFieldOpen(true)} className="inline-flex h-7 items-center gap-1 rounded-md border border-indigo-200 bg-surface px-2 text-[12px] font-semibold text-indigo-700 hover:bg-indigo-50"><Plus size={13} /> List field</button>
                  ) : (
                    <div className="flex flex-wrap items-center gap-1.5 rounded-lg border border-indigo-200 bg-indigo-50/40 px-2 py-1">
                      <input autoFocus value={nfName} onChange={(e) => setNfName(e.target.value)} placeholder="Field name" className="h-7 w-32 rounded-md border border-line bg-surface px-2 text-[12px] text-ink focus:border-indigo-400 focus:outline-none" />
                      <select value={nfType} onChange={(e) => setNfType(e.target.value as CrmAttribute['type'])} className="h-7 rounded-md border border-line bg-surface px-1.5 text-[12px] text-ink focus:outline-none">
                        {['TEXT', 'LONG_TEXT', 'NUMBER', 'CURRENCY', 'SELECT', 'MULTI_SELECT', 'DATE', 'DATETIME', 'BOOLEAN', 'USER', 'EMAIL', 'PHONE', 'URL'].map((t) => <option key={t} value={t}>{t.replace('_', ' ')}</option>)}
                      </select>
                      {(nfType === 'SELECT' || nfType === 'MULTI_SELECT') && (
                        <input value={nfOptions} onChange={(e) => setNfOptions(e.target.value)} placeholder="Options, comma-separated" className="h-7 w-44 rounded-md border border-line bg-surface px-2 text-[12px] text-ink focus:border-indigo-400 focus:outline-none" />
                      )}
                      <button type="button" disabled={nfSaving || !nfName.trim()} onClick={() => void addListField()} className="inline-flex h-7 items-center gap-1 rounded-md bg-indigo-600 px-2 text-[12px] font-semibold text-white hover:bg-indigo-700 disabled:opacity-50">{nfSaving ? <Loader2 size={12} className="animate-spin" /> : <Check size={12} />} Add</button>
                      <button type="button" onClick={() => { setAddFieldOpen(false); setNfName(''); setNfOptions(''); }} className="inline-flex h-7 items-center rounded-md border border-line px-2 text-[12px] font-semibold text-ink-muted hover:bg-surface-2">Cancel</button>
                    </div>
                  )}
                  {listFields.length > 0 && (
                    <button type="button" onClick={() => setManageFieldsOpen(true)} title="Rename, reorder, archive list fields and edit their options" className="inline-flex h-7 items-center gap-1 rounded-md border border-line bg-surface px-2 text-[12px] font-semibold text-ink-muted hover:bg-surface-2"><Settings2 size={13} /> Manage fields</button>
                  )}
                </div>
              )}
              <div className="overflow-hidden rounded-2xl border border-line bg-surface shadow-sm">
                <div className="grid items-center gap-3 border-b border-line bg-surface-2/40 px-4 py-2 text-[10.5px] font-bold uppercase tracking-[0.06em] text-ink-subtle" style={{ gridTemplateColumns: `minmax(0,1.4fr) ${previewKeys.map(() => 'minmax(0,1fr)').join(' ')}${showListFields ? ' ' + listFields.map(() => 'minmax(0,1.1fr)').join(' ') : ''} auto` }}>
                  ...
                  {showListFields && listFields.map((a) => (
                    <span key={a.id} className="inline-flex min-w-0 items-center gap-1 truncate text-indigo-600" title={`${a.name} · list field (lives on the list, not the record)`}>
      ...
      {manageFieldsOpen && detail && (
        <ManageListFieldsModal
          ...
          onSaved={async () => { setManageFieldsOpen(false); await loadDetail(detail.list.id); showToast('List fields updated'); }}
          onError={showToast}
        />
      )}
      ...
      {importOpen && detail && !isDynamic && (
        <ImportModal
          ...
          objectLabel={detail.list.primaryObject?.singularName ?? 'Records'}
          attrs={listAttrs}
          listTarget={{ listId: detail.list.id, listLabel: detail.list.name, listFields }}
          onClose={() => setImportOpen(false)}
          onImported={async (r) => { setImportOpen(false); await refresh(); showToast(`Imported · ${r.created} created, ${r.updated} updated`); }}
        />
      )}
  ...
  return (
    <div className="relative" ref={ref}>
      <button type="button" onClick={() => setOpen((o) => !o)} className={['inline-flex h-8 items-center gap-1.5 rounded-lg border px-2.5 text-[12.5px] font-medium transition-colors', active ? 'border-brand-400 bg-brand-50 text-brand-700' : 'border-line bg-surface text-ink-muted hover:bg-surface-2'].join(' ')}>
        <ArrowDownUp size={13} /> Sort{active ? ` · ${sorts.length}` : ''}
      </button>
      {open && (
        <div className="absolute right-0 top-[calc(100%+6px)] z-40 w-[320px] rounded-xl border border-line bg-surface p-2.5 shadow-xl">
          <p className="mb-1.5 text-[10px] font-bold uppercase tracking-[0.06em] text-ink-subtle">Sort · <span className="text-indigo-600">◇ = list field</span></p>
          {sorts.length === 0 && <p className="px-1 py-1.5 text-[12px] text-ink-subtle">No sorting yet — add a column below.</p>}
          ...
          <div className="mt-1.5 flex items-center gap-2">
            {addable.length > 0 && (
              <button type="button" onClick={add} className="inline-flex h-7 items-center gap-1 rounded-md border border-dashed border-line px-2 text-[11.5px] font-semibold text-brand-700 hover:bg-brand-50"><Plus size={12} /> Add sort</button>
            )}
            {active && <button type="button" onClick={() => onChange([])} className="text-[11px] font-medium text-ink-subtle hover:text-rose-600">Clear</button>}
          </div>
        </div>
      )}
    </div>
...
function ManageListFieldsModal({ listId, fields, onClose, onSaved, onError }: {
  ...
  const isSel = (t: CrmAttribute['type']) => t === 'SELECT' || t === 'MULTI_SELECT';
  ...
  function addOpt(i: number) { setRows((rs) => rs.map((r, j) => { if (j !== i) return r; const used = new Set(r.options.map((o) => o.value)); let v = 'option'; for (let n = 1; used.has(v); n++) v = `option_${n}`; return { ...r, options: [...r.options, { value: v, label: 'New option', color: null }] }; })); }
  ...
  async function save() {
    for (const r of rows) {
      ...
      if (!r.name.trim()) { onError('Every field needs a name'); return; }
      if (isSel(r.type) && r.options.some((o) => !o.label.trim())) { onError('Every option needs a label'); return; }
    }
    setBusy(true);
    try {
      ...
      const ax = e as { response?: { data?: { error?: string } } };
      onError(ax?.response?.data?.error ?? 'Could not save list fields');
    } finally { setBusy(false); }
  }

  return (
    <Modal open onClose={onClose} title="List fields" icon={<Tag size={14} />}
      footer={
        <>
          <button type="button" onClick={onClose} className="inline-flex h-9 items-center rounded-lg border border-line bg-surface px-3.5 text-[12.5px] font-medium text-ink-muted hover:bg-surface-2">Cancel</button>
          <button type="button" onClick={save} disabled={busy} className="brand-gradient inline-flex h-9 items-center gap-1.5 rounded-lg px-3.5 text-[12.5px] font-semibold text-white shadow-brand disabled:opacity-60">{busy ? <Loader2 size={13} className="animate-spin" /> : <Check size={13} />}Save fields</button>
        </>
      }
    >
      <div className="space-y-2">
        <p className="flex items-start gap-1.5 text-[11.5px] text-ink-muted"><Info size={12} className="mt-[1px] shrink-0 text-indigo-500" /><span>These fields live on the records-in-this-list (ListEntry), not on the records themselves. Reordering or renaming never changes the stored values; the field type can’t be changed once created.</span></p>
        {rows.map((r, i) => (
          <div key={r.id} className={['rounded-xl border px-2 py-2', r._delete ? 'border-rose-200 bg-rose-50/40' : 'border-line bg-surface'].join(' ')}>
            <div className="flex items-center gap-2">
              ...
              <span className="inline-flex shrink-0 items-center gap-1 text-indigo-500" title="List field"><Tag size={12} /></span>
              ...
              <span className="hidden shrink-0 rounded-md bg-surface-2 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-ink-subtle sm:inline" title="Field type (immutable)">{r.type.replace(/_/g, ' ')}</span>
              <label className={['inline-flex shrink-0 items-center gap-1 text-[11px] font-medium', r._delete ? 'text-ink-subtle' : 'text-ink-muted'].join(' ')} title="Required">
                <input type="checkbox" checked={r.isRequired} onChange={(e) => upd(i, { isRequired: e.target.checked })} disabled={!!r._delete} className="h-3.5 w-3.5 accent-indigo-600" /> Req
              </label>
              <button type="button" onClick={() => upd(i, { _delete: !r._delete })} title={r._delete ? 'Keep field' : 'Archive field'} className={['flex h-7 shrink-0 items-center justify-center rounded-md px-1.5 text-[11px] font-semibold', r._delete ? 'text-emerald-700 hover:bg-emerald-50' : 'text-ink-subtle hover:bg-rose-50 hover:text-rose-600'].join(' ')}>{r._delete ? 'Keep' : <Trash2 size={13} />}</button>
            </div>
            {isSel(r.type) && !r._delete && (
              <div className="mt-2 space-y-1 rounded-lg border border-indigo-100 bg-indigo-50/30 p-1.5">
                <p className="px-0.5 text-[10px] font-bold uppercase tracking-wide text-indigo-600">Options</p>
                {r.options.length === 0 && <p className="px-0.5 py-0.5 text-[11px] text-ink-subtle">No options yet.</p>}
                {r.options.map((o, oi) => (
                  <div key={oi} className="flex items-center gap-1.5">
                    <input value={o.label} onChange={(e) => setOptLabel(i, oi, e.target.value)} placeholder="Option label" className="h-7 min-w-0 flex-1 rounded-md border border-line bg-surface px-2 text-[12px] text-ink focus:border-indigo-400 focus:outline-none" />
                    <span className="hidden shrink-0 font-mono text-[9.5px] text-ink-subtle sm:inline" title="Stable value">{o.value}</span>
                ...
                <button type="button" onClick={() => addOpt(i)} className="inline-flex h-6 items-center gap-1 rounded-md border border-dashed border-indigo-200 px-1.5 text-[11px] font-semibold text-indigo-700 hover:bg-indigo-50"><Plus size={11} /> Option</button>
              </div>
            )}
          </div>
        ))}
        ...
        <div className="mt-1 rounded-xl border border-dashed border-indigo-200 bg-indigo-50/30 p-2">
          <p className="mb-1.5 inline-flex items-center gap-1 text-[10.5px] font-bold uppercase tracking-wide text-indigo-700"><Plus size={12} /> Add a list field</p>
          <div className="flex flex-wrap items-center gap-1.5">
            <input value={naName} onChange={(e) => setNaName(e.target.value)} placeholder="Field name" className="h-7 w-36 rounded-md border border-line bg-surface px-2 text-[12px] text-ink focus:border-indigo-400 focus:outline-none" />
            <select value={naType} onChange={(e) => setNaType(e.target.value as CrmAttribute['type'])} className="h-7 rounded-md border border-line bg-surface px-1.5 text-[12px] text-ink focus:outline-none">
              {LIST_FIELD_TYPES.map((t) => <option key={t} value={t}>{t.replace(/_/g, ' ')}</option>)}
            </select>
            {isSel(naType) && (
              <input value={naOptions} onChange={(e) => setNaOptions(e.target.value)} placeholder="Options, comma-separated" className="h-7 w-48 rounded-md border border-line bg-surface px-2 text-[12px] text-ink focus:border-indigo-400 focus:outline-none" />
            )}
          </div>
          <p className="mt-1 text-[10.5px] text-ink-subtle">Leave the name empty to add nothing. The new field is created on Save.</p>
        </div>
      </div>
    </Modal>
  );
